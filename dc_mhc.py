...
report_name = "ADC-MHC-Test_Court_Audit.xlsx"
# ----------------------------------------------DONT CHANGE ANYTHING BEYOND THIS POINT---------------------------------------------------


# -------------------------------Declare Queries----------------------------------
caselist_str = ["Age at Screening", "Current Age", "Days From Acceptance to SA Treatment",
                "Sentence/Disposition at Screening",
                "Primary Offense Charge Code", "Days From Arrest to SA Treatment", "Days In Jail",
                "Days In Jail By Bench Warrant",
                "Days In Jail By Drug Court Sanction", "Days In Jail By New Offense", "Days In Jail By Pre-Admission",
                "Days of 12 Step Program",
                "Days Rearrested in Program",
                "Drug Court Control Group", "First Treatment Date", "History of IV Use",
                "In SA Treatment Program At Screening", "IV Drug User",
                ...
                "Days Rearrested Post Program", "Number of Days in Phase I", "Number of Days in Phase II",
                "Number of Days in Phase III",
                "Number of Days in Phase IV", "Number of Days in Phase IV",
                "Number of Felonies", "CaseStatus", "Number of Incentives",
                "Number of Monitoring with Contact Made - No Violation",
                ...
                "Number of Scheduled Drug Court Reviews",
                "Percent of Positive Drug Tests", "Positive Drug Test In Program",
                "1st Positive Drug Test (In Program)", "Reoffend Status Offense In Program",
                "Reoffend Status Offense Post Program",
                "Retention Days", "Deceased Date", "Number Of Ancillary Sessions",
                "Number of Sessions", "SA Early Intervention/Education", "SA Intensive Outpatient", "SA Outpatient",
                "SA Outpatient Detox",
                "SA Residential", "SA Sub Acute Detox",
                "SA Treatment Contact Hours", "Qualifying Sentencing Score", "Rearrest Date in Program",
                "Rearrest Date Post Program",
                "Rearrest in Program", "Case Management/Support Coordination Days",
                "Co-occurring Treatment Service Days", "MH Treatment Contact Hours",
                "MH - Doctor/Medication Review Hours",
                "MH - Assertive Community Treatment (ACT) Hours",
                ...
                "Improvement in Quality of Life", "Prior Substance Abuse",
                "Substance Abuse Disorder at Screening", "Zip", "Recommended Treatment Modality At Screening",
...
caseinformation_str = ["ScreeningID", "LastName", "FirstName", "MiddleName", "DOB", "DrugCourtApproach",
                       "PrimaryOffense",
                       "Counts", "OffenseCategory", "OffenseType", "OffenseDate", "IncidentOffense",
                       "ArrestDate", "FirstAppearanceDate", "ArraignmentDate", "PreliminaryHearingDate",
                       "PretrialHearingDate", "CustodyStatus",
                       "BloodAlcoholContent", "IsWeaponUsed", "AdjudicationDate", "ProbationExpirationDate", "Sentence",
                       "JailStatus", "JailAdmitDate", "JailReleasedDate"]
...
screeningassessment_str = ["ScreeningID", "LastName", "FirstName", "Gender", "Ethnicity", "MiddleName", "DOB",
                           "ScreeningCaseManager",
                           "SecondaryDrugOfChoice", "TertiaryDrugOfChoice", "Comment",
...
ancillary_str = [ "CaseManager","FullName", "Judge", "Race","DOB", "Gender", "CaseStatus",  "ServiceUnits", "ContactHours", "ServiceFor","Sessions", "MissedSessions"
]

drugtest_str = [ "Clerk Case Number", "FullName", "Judge", "CaseManager"]

incentive_str = ["Clerk Case Number", "AdmissionDate","FullName","CasePhase","Gender", "Notes", "Details", "DOB",
                 "Judge", "Ethnicity-HispanicLatinoOrSpanishOrigin","CaseManager"]

jail_str = [ "Clerk Case Number", "FullName", "Gender", "Race", "DOB", "CaseManager", "Judge",
             "CaseStatus", "JailReason"]

suspension_str = ["Clerk Case Number", "CaseStatus","DaysSuspended", "ActivatedDate","CaseManager","Race","AdmissionDate",
                "FullName","CasePhase","Gender", "DischargeDate", "DischargeReason","DOB", "Judge"]

# ------------------call base functions and get reports-----------------------------------
caselist_report = caselist(caselist_str, start, end)
caseinfo_report = caseinfo(caseinformation_str, start, end)
...
jail_report = jail(jail_str)
suspension_report = suspension(suspension_str)
...
caseinfo_report.to_excel(writer, index=False, sheet_name="caseinformation")
...
jail_report.to_excel(writer,index=False, sheet_name= 'jailreport')
suspension_report.to_excel(writer,index=False, sheet_name= 'suspendedreport')
workbook = writer.book
redfill = workbook.add_format({'bg_color': '#FFC7CE'})
fill_list = ['caselist', "caseinformation", 'othercourtinvolvement', "screeningassessment", 'diagnosis',
             'relatedparties', "screening-military", "military", 'mentalhealth', 'familyinformation','pregnancy',
             'treatmentattendance','ancillaryprogress',"drugtests",'incentives', 'jailreport','suspendedreport']
for i in fill_list:
    formatter(i)
writer.save()
